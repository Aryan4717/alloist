...
def export_evidence(evidence_id: str, reason: str, output_path: str) -> bool:
    """Run export_evidence.py to produce signed evidence bundle."""
    script_dir = Path(__file__).resolve().parent
    export_script = script_dir / "export_evidence.py"
    try:
        subprocess.run(
            [
                sys.executable,
...
def main() -> int:
    ...
    if exit_code == 1:
        ...
        if evidence_id:
            ...
            if export_evidence(evidence_id, "Block large Stripe charges", evidence_path):

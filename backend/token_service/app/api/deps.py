...
from app.models import ApiKey, OrgRole, OrganizationUser, User
...
def get_current_user(
    ...
    Legacy TOKEN_SERVICE_API_KEY maps to default org + admin.
    """
    ...
    if settings.TOKEN_SERVICE_API_KEY and api_key == settings.TOKEN_SERVICE_API_KEY:
        return OrgContext(
...
def verify_api_key(
    ...
    """Verify API key from X-API-Key or Authorization: Bearer header. Legacy auth for routes that don't need RBAC."""
    api_key = get_settings().TOKEN_SERVICE_API_KEY
    if not api_key:
        raise HTTPException(

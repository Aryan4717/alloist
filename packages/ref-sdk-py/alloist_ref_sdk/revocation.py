...
def fetch_revocation_public_key(api_url: str) -> bytes | None:
    ...
    try:
        ...
        for key in data.get("keys", []):
            if key.get("kid") == REVOCATION_KID and key.get("x"):
                raw = _base64url_to_bytes(key["x"])
                _cached_key = (raw, now)
                return raw
        return _cached_key[0] if _cached_key else None
    except Exception:
...
def verify_revocation_payload(
    ...
    Provide public_key_b64 (base64 string) or public_key_bytes (raw 32 bytes).
    """
    if public_key_bytes is None and public_key_b64:
        try:
            public_key_bytes = base64.b64decode(public_key_b64.encode("ascii"))
        except Exception:
            return False
